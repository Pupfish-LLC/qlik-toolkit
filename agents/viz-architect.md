...
Per object: grid position (proportional or row/column range), object type (from the chart type decision framework), dimension and measure assignment (referencing expression catalog entries), calculation condition (when to evaluate, with user-facing message) and show condition (when to render at all), sorting and formatting rules, responsive behavior notes.

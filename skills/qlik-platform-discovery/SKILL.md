...
- **Parameters** (declared in the `SUB Name(p1, p2, ...)` line). Parameters are positional — order matters. Three behaviorally critical facts to record alongside the parameter list:
  - **Optional parameters via NULL initialization.** Per Tier-1 docs (https://help.qlik.com/en-US/cloud-services/Subsystems/Hub/Content/Sense_Hub/Scripting/ScriptControlStatements/Sub.htm): "If a subroutine has more formal parameters than actual parameters passed by a call statement, the extra parameters will be initialized to NULL and can be used as local variables within the subroutine." This means a SUB declared with `SUB MySub(pA, pB, pC)` can be called as `CALL MySub('x')` — `pB` and `pC` will be NULL inside the SUB. Inspect call sites to see which parameters are actually passed; trailing parameters that are never passed at any call site are de facto optional. Record them as "Optional — defaults to NULL; confirm caller intent."
  - **Copy-out semantics for variable-name actuals.** Per the same Tier-1 page: "Arguments are copied into the subroutine and, if the corresponding actual parameter in the call statement is a variable name, copied back out again upon exiting the subroutine." A SUB that mutates `pResult` will write the new value back to whatever caller-scope variable was passed in as that parameter. This is the intended mechanism for returning values from a SUB, but it also means caller-scope variables passed positionally can be silently modified — record copy-out behavior as a known characteristic of any SUB whose body assigns to a parameter. Literal arguments are not affected (there is no variable to copy back into).
  - **Parameter prefix convention.** There is no Qlik-mandated parameter prefix, but mature platforms commonly use `p` (e.g., `pTableName`, `pKeyField`) to distinguish formal parameters from globally-scoped script variables. If the platform's existing SUBs use a consistent prefix, record it alongside the `v` variable prefix as part of the platform's naming conventions.
...
- **Known limitations**: what the subroutine does NOT handle. These are rarely obvious from code alone — look for `WHERE`, `EXISTS`, and `CONCATENATE` patterns that assume single-field keys, hardcoded table names, or wildcard SELECTs that would inject phantom fields if called from a different context. Optional-parameter and copy-out behavior often belong here too (e.g., "ParameterN is optional; default is NULL — confirm caller behavior" or "Writes back to caller-scope variable passed as pResult"). When a limitation isn't testable from the code, flag it as a question for the developer rather than assuming.
- **Call sites**: every `CALL SubName(...)` reference in the broader script base. Call sites show the operational signatures developers actually use, which sometimes diverge from the declared parameter intent. They are also the primary signal for which formal parameters are actually exercised and which are de facto optional.
